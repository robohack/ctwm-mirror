...
TWM_TYPE::
  ...
  string ``ctwm'' in this program.  This could be useful for
  protecting parts of your `.twmrc` file that `twm` proper won't
  understand (like `WorkSpaces`) so that it is still usable with other
  `twm`-like programs, especially those supporting M4 pre-processing
  (for `twm` itself it would be necessary to use M4's include file
  mechanisms to keep `ctwm`-only configuration in a separate file).
  Note that `ctwm` will fall back to reading `.twmrc`.

TWM_VERSION::
  Tells which ctwm version is running.  It is in the form of:
...
String arguments that are interpreted as filenames (including colon-separated paths; see the `Pixmaps`,
`Cursors`, `IconDirectory`, and `PixmapDirectory` below) will
...
file.  Lists of window name / window class extended regular
expressions are indicated by `win-list`.  Optional arguments are shown
in square brackets:
...
BorderColor `string` [{ `wincolorlist` }]::
  ...
  of window name or class extended regular expressions and color name
  pairs for specifying particular border colors for different types of
  windows.
  For example:
+
------
...
f.startwm `string`::
  This function shuts down ctwm and execs an alternate window manager in its
  place.  The argument `string` is passed to `/bin/sh` for execution.
+
First appeared in 4.2.0.
...
events `NoTitleFocus` is set), unless `NoIconManagerFocus` is set.
